function createUnityInstance(canvas, config, onProgress = () => { }) {
  ...
  var unityInstance = {
    ...
    GetMetricsInfo: function () {
      const metricsInfoPtr = Number(Module._GetMetricsInfo()) >>> 0;
      var totalWASMHeapSizePtr = metricsInfoPtr;
      var usedWASMHeapSizePtr = totalWASMHeapSizePtr + 8;
      var totalJSHeapSizePtr = usedWASMHeapSizePtr + 8;
      var usedJSHeapSizePtr = totalJSHeapSizePtr + 8;
      ...
      return {
        totalWASMHeapSize: Module.HEAPF64[totalWASMHeapSizePtr >> 3],
        usedWASMHeapSize: Module.HEAPF64[usedWASMHeapSizePtr >> 3],
        ...
        numJankedFrames: Module.HEAPF64[numJankedFramesPtr >> 3]
      };
    }
  };
  ...
  Module.SystemInfo = (function () {
    ...
    const hasMemory64 = (() => {
      try {
        return WebAssembly.validate(new Uint8Array([0,97,115,109,1,0,0,0,5,8,1,5,128,128,4,128,128,16,7,7,1,3,109,101,109,2,0]));
      } catch(_) {}
      return false;
    })();
  ...
  function progressUpdate(url, e) {
    var progress = Module.downloadProgress[url] || createDownloadProgressMetadata(url);
    ...
    // Files whose loading is deferred until after engine initialization shouldn't trigger the onProgress callback.
    if (progress.loadingDeferred) {
      return;
    }

    // Since we can't compute the time needed to compile WASM and start the player, we assume these final steps account
    // for about 10% of the total loading time.
    const PROGRESS_COEFFICIENT = 0.9;
    const progressByFile = Object.values(Module.downloadProgress).filter(fileProgress => !fileProgress.loadingDeferred);
    onProgress(PROGRESS_COEFFICIENT * computeTotalDownloadProgress(progressByFile));
  }

  function computeTotalDownloadProgress(progressByFile) {
    const fileCount = progressByFile.length;
    if (fileCount === 0 || progressByFile.some(fileProgress => !fileProgress.started)) {
      return 0;
    }
    ...
    } = progressByFile.reduce((acc, fileProgress) => {
      if (fileProgress.lengthComputable) {
        acc.loaded += fileProgress.loaded;
        acc.total += fileProgress.total;
        acc.computable++;
      } else if (!fileProgress.finished) {
        acc.unfinishedNonComputable++;
      }
      return acc;
    }, {
      loaded: 0,
    ...
    const percentLoaded = (total - loaded) / total;
    const remainingComputable = (
      total
      ? computable * percentLoaded
      : 0
    );

    return (fileCount - unfinishedNonComputable - remainingComputable) / fileCount;
  }
...
Module.readBodyWithProgress = function() {
  ...
  function readBodyWithProgress(response, onProgress, enableStreaming) {
    ...
    var lengthComputable = response.headers.get('Content-Length') !== null;
    ...
    function readBody() {
      if (typeof reader === "undefined") {
        ...
        // Fallback to Response.arrayBuffer()
...
Module.fetchWithProgress = function () {
  function fetchWithProgress(resource, init) {
    ...
    return fetch(resource, init).then(function (response) {
      return Module.readBodyWithProgress(response, onProgress, init && init.enableStreamingDownload);
    });
  }
  ...
  function createDownloadProgressMetadata(url, loadingDeferred = false) {
    const existingProgress = Module.downloadProgress[url];
    if (existingProgress)
      return existingProgress;

    return Module.downloadProgress[url] = {
      loadingDeferred,
      started: false,
      finished: false,
  ...
  function downloadBinary(url, loadingDeferred = false) {
      createDownloadProgressMetadata(url, loadingDeferred);
      ...
      var request = fetchImpl(url, {
        ...
        mode: mode,
        onProgress: (event) => progressUpdate(url, event),
      });
  ...
  function downloadDataFile(url, loadingDeferred = false) {
    return downloadBinary(url, loadingDeferred).then((data) => ({url, data}));
  }

  function createDataFile({url, data}, simulateThrottingForTesting = false) {
     // Avoid storing compression extension in the virtual filesystem
    const path = url.replace(/\.(gz|br)$/, "");

    // Create the full path leading up to the target filename ("mkdir -d" behavior)
    for (let currentIndex = 0, nextIndex = path.indexOf("/", currentIndex) + 1;
         nextIndex > 0;
         currentIndex = nextIndex, nextIndex = path.indexOf("/", currentIndex) + 1) {
      Module.FS_createPath(path.substring(0, currentIndex), path.substring(currentIndex, nextIndex - 1), true, true);
    }
    Module.FS_createDataFile(path, null, data, true, true, true);

  }

  function downloadData() {
    var dataUrlLoadStartTime = performance.now();
    var dataPromise = downloadBinary(Module["dataUrl"]);
    Module.preRun.push(function () {
      ...
      dataPromise.then(function (data) {
        ...
        while (pos < headerSize) {
          ...
          pos += pathLength;
          createDataFile({url: path, data: data.subarray(offset, offset + size)});
        }
